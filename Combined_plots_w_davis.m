...
%% Plot settings
cols = ["#0072BD", "#D95319", "#EDB120", "#77AC30", "#4DBEEE"];  % Colors of the lines
markers = ["none", "+", "o", "diamond", "x"];  % Markers for the four methods
ms = [8, 8, 4.5, 6.5, 6.5];  % Marker size for the plots of the four methods
lw = [1.2, 1.2, 1, 1, 1];  % Linewidth for the lines of the four methods
...
if plot_C_l
    for i = 1:length(airfoil_names)
        ...
        i_panel_davis = find(strcmp({panel_res_davis.name}, airfoil_names(i)));
        ...
        plt_cl_panel_davis = plot(panel_res_davis(i_panel_davis).panel_res.alpha, ...
                                  panel_res_davis(i_panel_davis).panel_res.C_l, ...
        ...
        %Order the plots 
        ax.Children = [plt_cl_thin; plt_cl_panel; plt_cl_panel_davis; plt_cl_free; plt_cl_fixed; x_ax; y_ax];
        hold off; 
    
        % Plot labels
        set(gcf,'Color','White');
        set(ax,'FontSize',fs);
        legend([plt_cl_thin, plt_cl_panel, plt_cl_panel_davis, plt_cl_free, plt_cl_fixed], ...
...
if plot_C_p
    for i = 1:length(airfoil_names)
        ...
        plt_cp_panel = plot(panel_res(i).xp,panel_res(i).Cp, ...
                            LineWidth=lw(2), Marker=markers(2), MarkerSize=ms(2));
        plt_cp_panel_davis = plot(panel_res_davis(i_panel_davis).panel_res.x, ...
                                  panel_res_davis(i_panel_davis).panel_res.C_p(201,:), ...
                            LineWidth=lw(5), Marker=markers(5), MarkerSize=ms(5));
        ...
        %Order the plots 
        ax.Children = [plt_cp_panel; plt_cp_panel_davis; plt_cp_free; plt_cp_fixed; x_ax; y_ax];
        hold off; 
    
        % Plot labels
        set(gcf,'Color','White');
        set(ax,'FontSize',fs);
        legend([plt_cp_panel, plt_cp_panel_davis, plt_cp_free, plt_cp_fixed], ...
            {'Panel method', 'Panel method Davis', 'XFOIL (free transition)', ...
